class ProductSelectionModal {
    ...
    init() {
        console.log('ProductSelectionModal init() 시작');
        
        // 템플릿 로딩을 고려해서 약간의 지연을 두고 초기화
        setTimeout(() => {
    ...
    async getOptionsForStep(field) {
        try {
            console.log(`${field} 옵션 로딩 시작... (현재 단계: ${this.currentStep})`);
            ...
            const url = `/api/product-options/${encodeURIComponent(field)}?${params}`;
            ...
            console.log(`${field} 옵션 데이터:`, data);
            
            // 배열인지 확인
            if (Array.isArray(data)) {
                console.log(`${field} 옵션 개수:`, data.length);
                return data;
            } else {
                console.error(`${field} 옵션 데이터가 배열이 아님:`, typeof data, data);
                return [];
            }
        } catch (error) {
